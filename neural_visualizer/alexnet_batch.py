...
class AlexNet(nn.Module):
    def __init__(self, num_classes, standard=True, fc_connect=True):
        super(AlexNet, self).__init__()
        if standard:
            n_filters = [96, 256, 384, 384, 256]
        else:
            n_filters = [64, 192, 384, 256, 256]

        conv1 = nn.Sequential(
            nn.Conv2d(3, n_filters[0], kernel_size=11, stride=4, padding=2),
            nn.BatchNorm2d(n_filters[0]),
            nn.ReLU(inplace=True),
        )
        pool1 = nn.MaxPool2d(kernel_size=3, stride=2)
        conv2 = nn.Sequential(
            nn.Conv2d(n_filters[0], n_filters[1], kernel_size=5, padding=2),
    ...
    def forward(self, x, setting='default', **kargs):
        assert setting in ['default', 'layers', 'layer']
        if setting == 'default':
            for subnet in self._feature_blocks:
                x = subnet(x)
            return x
        elif setting == 'layers':
            return self.forward_as_feature(x, **kargs)
        elif setting == 'layer':
            return self.forward_to_layer(x, **kargs)

    def forward_to_layer(self, x, layer_name):
        assert layer_name in self.all_feat_names
        for idx, subnet in enumerate(self._feature_blocks):
            x = subnet(x)
            if self.all_feat_names[idx] == layer_name:
                break
        return x
    ...
    def forward_as_feature(self, x, out_feat_keys=None):
...
def alexnet(num_classes, standard=False, fc_connect=False):
    return AlexNet(num_classes, standard=standard, fc_connect=fc_connect)
